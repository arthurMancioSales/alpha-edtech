...
const { use } = require("../routers/orders");
...
async function addOrderDiscountCodeToOrderByCode(order_id, code, user_id) {
  ...
  if (!existingCode) {
    throw createError(
      "NotFoundError",
      `Code "${existingCode}" does not exists`
    );
  }
  
  if (existingCode.expiration_date <= new Date()) {
    throw createError(
      "ForbiddenError",
      `Code "${existingCode}" is expired`
    );
  }

  const orderItems = await orderItemsRepo.getOrderItemsWithProductInformationByOrderId(order_id)
  ...
  if (existingCode.minimum_order_value && orderTotalPrice < existingCode.minimum_order_value) {
    throw createError(
      "ForbiddenError",
  ...
  const validCategories = await applicableCategoriesRepo.getApplicableCategoriesByDiscountCodeId(existingCode.id)
  const validCategoriesId = validCategories.map((category) => category.id)
  
  const orderItemsCategories = orderItems.map((items) => items.product_category_id)

  const orderHasValidCategory = validCategoriesId.some((item) => orderItemsCategories.includes(item))
  ...
  const usedCodes = await orderDiscountsRepo.getOrderDiscountCodesByOrderId(order.id)

  if (usedCodes.length > 0) {
    throw createError(
      "ForbiddenError",
      `Code ${code} was already used`
    );
  }
  // await orderItemsRepo.updateOrderItem(order_id, product_id, quantity);
}
